...
# Note: First run will download CRAFT & recognition models (~130MB)
reader = easyocr.Reader(['en'], gpu=True)  # Use GPU acceleration
...
def box_points(box):
    """Convert EasyOCR box format to list of 4 points"""
...
try:
    ...
    print("\nStarting IMX298 Book Shelf Reading...")
    print("="*60)
    ...
    while True:
        ...
        if (now - last_ocr_t) >= OCR_PERIOD_S and len(frame_buffer) == FRAME_BUFFER_SIZE:
            ...
            if results:  # non-empty
                for result in results:
                    try:
                        # EasyOCR format: ([[p1],[p2],[p3],[p4]], text, confidence)
                        box = result[0]  # list of 4 points
                        text = result[1]  # string
                        conf = result[2]  # float confidence
                        
                        text = str(text).strip().upper()
                        if not text:
                            continue
                        
                        # Convert box to proper list of (x, y)
                        ...
                        print(f"Skipping a detection due to error: {e}")
                        continue
            ...
            last_ocr_t = now
            print(f"  Detected {len(cached_clusters)} books:")
        ...
        y_offset = 50
        cv2.putText(disp,"Shelf Order (Left to Right):",(20,y_offset),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),2)
        for i, (call, _, _) in enumerate(cached_call_numbers):
            y_offset += 35
            cv2.putText(disp,f"{i+1}. {call}",(20,y_offset),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),2)
        ...
        if now - t0 >= 1.0:
            ...
            t0 = now
            frame_count = 0
        cv2.putText(disp,f"FPS: {disp_fps:.1f} | Books: {len(cached_clusters)}",(COLOR_W-400,30),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,0,255),2)
        cv2.putText(disp,f"Buffer: {len(frame_buffer)}/{FRAME_BUFFER_SIZE}",(COLOR_W-400,70),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,0),2)
        ...
        if key in [27, ord('q')]:
            ...
            print(f"Captured frame: capture_{ts}.jpg")
