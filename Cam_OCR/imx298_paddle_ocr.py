...
def box_points(box):
    ...
    if len(box) == 4 and all(isinstance(p, (list, tuple)) for p in box):
        ...
        # Flat 4 floats: [x_min, y_min, x_max, y_max]
        x0, y0, x1, y1 = box
        return [(x0,y0), (x1,y0), (x1,y1), (x0,y1)]
    else:
        # Fallback: return 4 dummy points to avoid crash
...
try:
    ...
    while True:
        ...
        if (now - last_ocr_t) >= OCR_PERIOD_S and len(frame_buffer) == FRAME_BUFFER_SIZE:
            ...
            #result = ocr.predict(roi_proc, det=True, rec=True, cls=True)
            #result = ocr.predict(roi_proc)
            ...
            if result:  # non-empty
                for line in result:
                    ...
                    try:
                        box = line[0]  # always list of 4 points
                        text_conf = line[1]  # tuple/list: [text, confidence]

                        # Unpack safely
                        if isinstance(text_conf, (tuple, list)) and len(text_conf) >= 2:
                            text, conf = text_conf
                            # Conf might be a list, convert to float safely
                        ...
                        if isinstance(box[0], list) or isinstance(box[0], tuple):
                            ...
                            # Flattened box? assume 8 numbers
